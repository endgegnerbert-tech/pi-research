...
const BLOCKED_PATTERN = /attention required!|cloudflare|access denied|temporarily unavailable/i;
  ...
  return body
    ...
    .map((chunk, idx) => {
      ...
      return {
        ...
        blocked: BLOCKED_PATTERN.test(text),
...
export function extractConflictStructuredFeatures(row = {}) {
  ...
  return {
    ...baseStructuredFeatures(row.query || "", sources),
    candidate_conflict: String(row.candidateLabel || row.label || "").includes("conflict") ? 1 : 0,
