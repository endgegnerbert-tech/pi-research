...
import { classifyDomainWithLocalSlm, planQueriesWithLocalSlm } from "./local-slm.js";
...
export async function buildQueries(query, mode = "fast", ctx, signal) {
  ...
  try {
    const localPlan = await planQueriesWithLocalSlm(query, config.mode, config.maxQueries, signal);
    if (localPlan?.queries?.length) {
      await logResearchEvent("local_slm_query_plan", { mode: config.mode, intent: localPlan.intent, count: localPlan.queries.length });
      return [...new Set([...localPlan.queries, ...hintedQueries])].slice(0, config.maxQueries);
    }
  } catch {
    // keep heuristic/cloud fallback reliable when local inference is unavailable
  }
...
import { classifyDomainWithTinyRouter, classifyFollowupWithTinyRouter } from "./tiny-router.js";

async function resolveQuestionDomain(query, mode, signal) {
  const fallback = classifyQuestionDomain(query);
  try {
    const tinyDomain = await classifyDomainWithTinyRouter(query, mode, signal);
    if (tinyDomain) return tinyDomain;
